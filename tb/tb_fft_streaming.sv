    ...
    for (int k = 0; k < FFT_LEN; k++) begin
      ...
      if (out_valid) begin
        if (out_real < 16'sh7FFE || out_real > 16'sh7FFF) begin
          $fatal(1, "FFT streaming output mismatch at bin %0d: %0d", k, out_real);
        end
        if (out_imag != 16'sd0) begin
          $fatal(1, "FFT streaming output imag mismatch at bin %0d: %0d", k, out_imag);
        end
      end
    end
    $display("FFT streaming test passed.");
    $finish;
  end
endmodule
